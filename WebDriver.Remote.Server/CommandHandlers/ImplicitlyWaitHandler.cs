// <copyright file="ImplicitlyWaitHandler.cs" company="WebDriver Committers">
...
{
    /// <summary>
    /// Provides the handler for the <see cref="DriverCommand.ImplicitlyWait"/> command.
    /// </summary>
    internal class ImplicitlyWaitHandler : WebDriverCommandHandler
    {
        ...
        {
            return string.Format(CultureInfo.InvariantCulture, "[implicitly wait: {0}]", this.milliseconds);
        }
        ...
        {
            Session.Driver.Manage().Timeouts().ImplicitlyWait(TimeSpan.FromMilliseconds(this.milliseconds));
            return null;
        }
    }
}
